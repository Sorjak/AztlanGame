...
namespace PlatformerPro
{
	...
	public class WallMovement_LedgeClimb : WallMovement
	{
		...
		/// <summary>
		/// Stores the ledge position for the current ledge.
		/// </summary>
		...
		override public void DoMove()
		{
			...
			if (ledgeClimbState == LedgeClimbState.REACHING && CheckForGrab()) ledgeClimbState = LedgeClimbState.GRASPING;
		}
		...
		virtual protected bool CheckLedge() 
		{
			switch (ledgeDetectionType)
			{
				case LedgeDetectionType.NONE:
					...
					{
						currentLedgePosition = character.CurrentWall.transform.position;
						return true;
					}
					break;
				case LedgeDetectionType.BOX:
					if (character.CurrentWall && character.CurrentWall is BoxCollider2D) 
					{
						...
						if (facingDirection == 1)
						{
							...
							currentLedgePosition = new Vector2(-ledgeOffset.x, ledgeOffset.y) + 
				...
				case LedgeDetectionType.CIRCLE_CAST:
					...
					{
						currentLedgePosition = new Vector2(-ledgeOffset.x, ledgeOffset.y) + ledgeHit.point;
						return true;
					}
					break;
			}
